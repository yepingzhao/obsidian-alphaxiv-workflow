"""
Fetch hot/trending papers from AlphaXiv main page.
Scrapes arxiv IDs from the server-rendered HTML, then enriches via batch arXiv API.

Usage:
    python fetch_hot_papers.py                    # PrettyTable display
    python fetch_hot_papers.py --json             # JSON output for piping
    python fetch_hot_papers.py --limit 10         # Top N only
    python fetch_hot_papers.py /path/to/vault     # With vault duplicate detection
    python fetch_hot_papers.py /path/to/vault --data-file /tmp/hot_papers.json
"""
import asyncio
import json
import os
...
ALPHAXIV_URL = 'https://alphaxiv.org/'
ARXIV_ID_RE = re.compile(r'(\d{4}\.\d{4,5})')


def fetch_hot_ids() -> list:
    """Scrape arxiv IDs from AlphaXiv main page HTML.

    The main page is server-rendered and contains paper IDs in the HTML.
    Returns deduplicated, sorted list of arxiv IDs.
    """
    req = urllib.request.Request(
        ALPHAXIV_URL,
        headers={'User-Agent': 'AlphaXivToObsidian/1.0 (hot-papers)'}
    )
    try:
        with urllib.request.urlopen(req, timeout=15) as r:
            html = r.read().decode('utf-8')
    except (urllib.error.URLError, urllib.error.HTTPError, OSError) as e:
        print(f'❌ Failed to fetch {ALPHAXIV_URL}: {e}', file=sys.stderr)
        sys.exit(1)

    ids = sorted(set(ARXIV_ID_RE.findall(html)))
    if not ids:
        print('❌ No arxiv IDs found on AlphaXiv main page.', file=sys.stderr)
        sys.exit(1)
...
def enrich_hot_papers(arxiv_ids: list, vault_path: str = None) -> list:
    """Enrich hot paper IDs with arXiv metadata.
    ...
    for aid in arxiv_ids:
        ...
        # Quality rating without CCF (no EasyScholar calls for hot page — fast display)
        quality = {}
...
def build_table(results: list) -> PrettyTable:
    """Build PrettyTable for hot papers display."""
    table = PrettyTable()
    table.field_names = [
...
def export_json(results: list) -> list:
    """Export hot papers as JSON-serializable list for AI handoff."""
    output = []
    for p in results:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Fetch hot/trending papers from AlphaXiv main page'
    )
    parser.add_argument(
    ...
    # Step 1: Scrape IDs
    print('🔥 获取 AlphaXiv 热门论文...', file=sys.stderr)
    ids = fetch_hot_ids()
    if args.limit and args.limit > 0:
        ids = ids[:args.limit]
    print(f'🔥 获取到 {len(ids)} 篇热门论文', file=sys.stderr)
    ...
    results = enrich_hot_papers(ids, args.vault_path)
    ...
    print(table)
    print()
    print('🔥 AlphaXiv 热门论文 — 基于社区关注度和讨论热度排序')
