...
from alphaxiv_client import get_paper_metadata

from config import PAPERS_DIR
PAPERS = PAPERS_DIR

REQUIRED = ['title', 'arxiv_id', 'version', 'date', 'tags', 'source', 'authors', 'aliases', 'created']
OPTIONAL = ['published_venue', 'published_date', 'presentation_type', 'ccf',
            'sci_jcr', 'sci_cas', 'sci_cas_top', 'sci_cas_small',
            'fms', 'utd24', 'ft50', 'swufe', 'blog_status']
...
def normalize_note(fp):
    ...
    for k, v in new.items():
        if isinstance(v, list):
            ...
            if v.lower() in ('true', 'false') or v.replace('.','').replace('-','').replace('/','').isdigit():
