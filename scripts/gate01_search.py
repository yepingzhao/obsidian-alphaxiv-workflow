...
Usage:
    python gate1_search.py "diffusion models" /path/to/vault
    python gate1_search.py "diffusion models" /path/to/vault --limit 5
    python gate1_search.py "diffusion models" /path/to/vault --json  # machine output
...
async def main_async(query: str, vault_path: str, limit: int = 10):
    ...
    log(f'🔍 搜索中: "{query}"')
    t0 = datetime.now()
    results = list(search_papers(query, limit=limit))
    if not results:
