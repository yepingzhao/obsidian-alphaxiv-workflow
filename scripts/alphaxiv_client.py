"""
Shared AlphaXiv API client - search, metadata, and overview fetching.
Used by all gates in the alphaxiv-to-obsidian pipeline.
"""
import os
import re
import time
...
try:
    from publication_rank import get_venue_ranking, extract_ranking_fields
except ImportError:
    get_venue_ranking = None  # type: ignore
    extract_ranking_fields = None  # type: ignore
...
# ──────────────────────────────────────────────────────────────────
# Publication info — venue, oral/spotlight, formal publication date
# Fetched from arXiv API (journal_ref + comment fields)
# ──────────────────────────────────────────────────────────────────
...
def fetch_publication_info(arxiv_id: str, abstract: str = None) -> dict:
    ...
    try:
        ...
        # How venue was found (for transparency)
        venue_source = 'journal_ref' if result['published_venue'] else None
...
def fetch_publication_info_batch(arxiv_ids: list, abstracts: dict = None) -> dict:
    ...
    Returns:
        dict keyed by arxiv_id, each value is the same shape as fetch_publication_info()
    """
...
def check_single_duplicate(vault_path: str, arxiv_id: str) -> str | None:
    """Quick check: does a single arXiv ID exist in vault? Returns filepath or None."""
    return check_vault_for_papers(vault_path, [arxiv_id]).get(arxiv_id)
