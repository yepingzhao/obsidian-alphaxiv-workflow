...
| Input | Mode | CLI |
|-------|------|-----|
| `analyze "topic"` | Topic | `<PYTHON_CMD> -m alphaxiv_workflow.synthesis topic "TOPIC" "VAULT"` |
| `analyze author "name"` | Author | `<PYTHON_CMD> -m alphaxiv_workflow.synthesis author "NAME" "VAULT"` |

### Step 2: Search Vault

```bash
<PYTHON_CMD> -m alphaxiv_workflow.synthesis topic "TOPIC" "VAULT_PATH"
```
...
# Save the scaffold, then pass the prompt to the current model
```

### Step 5: LLM Synthesis Generation

Pass `prompt` to the current model. Generate five chapters in Chinese with wikilink citations:
...
- Natural language: "方法对比太浅，每组的优缺点写详细些" → rewrite the relevant chapter
